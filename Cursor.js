...
var Cursor=(b)=>({// Buffer -> Cursor
    ...
    to_eol(){if(!this.eol()){this.right(b.getline(this.cl).length-this.co-1);}},
    ...
    forward_word(){var ra=b.s.slice(b.pt+1).search(/\w\W/);if(ra>=0){this.right(ra+1,true);}},
    backward_word(){var ra=b.s.slice(0,b.pt).split('').reverse().join('').search(/(\w\W)|(\W)/);if(ra>=0){this.left(ra+1,true);}},
    forward_to_char(c){if(this.eol()){return;}var ca=b.getline(this.cl).slice(this.co+1).indexOf(c);if(ca>=0){this.right(ca+1);}},
    backward_to_char(c){
        if(this.bol()){return;}
    ...
    // Motions : Int (and possibly also a Bool) -> () // move the Point
    left(n,freely=false){b.pt-=n;if(b.pt<0){b.pt=0;}if(!freely&&n===1&&b.s[b.pt]==='\n'){b.pt+=1;}this.rowcol();},
    ...
    up_down_helper(target_line){
        ...
        this.co=Math.min(Math.max(0,target_line_length),this.cx);
        this.cl=target_line;
        if(target_line===0){b.pt=this.co;}
        else{b.pt=b.lines[target_line]+1+this.co;}
    },
    rowcol(){this.cl=b.pt?this.curln():0;this.cx=this.co=b.pt-(!this.cl?0:1)-b.lines[this.cl];},// move Point
